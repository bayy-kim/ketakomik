...
import { Flame, Droplet, Volume2, VolumeX, BookOpen, Swords, Trophy, Sparkles, Play, User, LogIn, UserPlus } from "lucide-react";
      ...
      <div className="max-w-6xl mx-auto px-2.5 sm:px-3 py-2 flex flex-col sm:flex-row items-center justify-between gap-2">
        ...
        <nav className="w-full sm:w-auto flex items-center justify-around sm:justify-end gap-1 sm:gap-2 pt-1 sm:pt-0 border-t-2 sm:border-t-0 border-gray-200">
          {isLoggedIn ? (
            ...
            /* Public Guest Header — NO Game or Admin Links Shown */
            <>
              <Link
                href="/auth/login"
                className="flex-1 sm:flex-initial min-h-[38px] px-3 py-1 bg-white comic-border-sm rounded hover:bg-gray-100 transition-colors comic-shadow-sm flex items-center justify-center gap-1 font-bangers text-xs sm:text-sm text-comic-ink"
              >
                <LogIn className="w-3.5 h-3.5 text-comic-ink" />
                <span>MASUK</span>
              </Link>
              <Link
                href="/auth/login"
                className="flex-1 sm:flex-initial min-h-[38px] px-3.5 py-1 bg-comic-yellow comic-border-sm rounded hover:bg-yellow-400 transition-colors comic-shadow-sm flex items-center justify-center gap-1 font-bangers text-xs sm:text-sm text-comic-ink"
              >
                <UserPlus className="w-3.5 h-3.5 text-comic-ink" />
