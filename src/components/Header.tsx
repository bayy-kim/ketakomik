...
import { Flame, Droplet, Volume2, VolumeX, BookOpen, Swords, Trophy, Sparkles, LogIn, Lock, Play, LayoutDashboard, WifiOff } from "lucide-react";
...
export function Header({ mode: initialMode, onModeToggle, tintaCount: propTinta, streakCount: propStreak }: HeaderProps) {
  ...
  const isOfflinePage = pathname === "/play-offline";
  ...
  return (
    <header className="w-full bg-white border-b-4 border-comic-ink shadow-[0_4px_0_#16161A] sticky top-0 z-40">
      <div className="max-w-6xl mx-auto px-2.5 sm:px-3 py-2 flex flex-col sm:flex-row items-center justify-between gap-2.5">
        ...
        <div className="w-full sm:w-auto flex items-center justify-between gap-2.5">
          ...
          <Link href="/" className="flex items-center gap-1.5 shrink-0 group">
            <div className="bg-comic-yellow comic-border px-2.5 py-1 rounded-md rotate-[-2deg] group-hover:rotate-0 transition-transform comic-shadow-sm flex items-center gap-1">
              ...
              <span className="font-bangers text-xl sm:text-2xl text-comic-ink tracking-wider ml-0.5 inline">TEKAKOMIK</span>
            </div>
          </Link>

          {/* If Offline Page: Show Offline Badge & Hide Online Links */}
          {isOfflinePage ? (
            <div className="flex items-center gap-1.5 bg-comic-bayangan text-white comic-border px-3 py-1 rounded-md comic-shadow-sm font-bangers text-xs sm:text-sm">
              <WifiOff className="w-4 h-4 text-comic-yellow animate-pulse" />
        ...
        {/* Navigation Bar (Hidden in Offline Page) */}
        {!isOfflinePage && (
