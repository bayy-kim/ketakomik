...
import { AlertTriangle, TrendingUp, Users, Award, Database } from "lucide-react";
...
export default function AdminAnalyticsPage() {
  ...
  if (loading) {
    return <div className="p-8 font-bangers text-2xl text-comic-ink">MEMUAT GRAFIK ANALITIK HARIAN...</div>;
  }
    ...
    <div className="flex flex-col gap-6 max-w-5xl">
      <div className="bg-white comic-border p-4 rounded-xl comic-shadow flex items-center justify-between">
        <div>
          ...
          <p className="text-xs font-sans text-gray-700">
            Pantau DAU/WAU, distribusi mode, serta kata yang terdeteksi terlalu mudah atau terlalu sulit secara real-time.
          </p>
        </div>
      </div>
      ...
      {!hasData ? (
        <div className="bg-white comic-border p-8 rounded-xl comic-shadow text-center flex flex-col items-center gap-3">
          ...
          <h3 className="font-bangers text-2xl text-comic-ink">BELUM ADA DATA PERMAINAN</h3>
          <p className="text-xs font-sans text-gray-600 max-w-md">
            Statistik analitik dan grafik Recharts akan otomatis terisi setelah pemain mulai menyelesaikan tebak kata harian!
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="bg-amber-50 comic-border p-4 rounded-xl comic-shadow flex flex-col gap-3">
            ...
            {data.flaggedWords.length === 0 ? (
              <p className="text-xs font-sans text-gray-500 italic">Belum ada kata yang terdeteksi anomali kesulitan.</p>
            ) : (
